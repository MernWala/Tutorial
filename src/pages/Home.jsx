...
const Home = ({ setModalShow, myString, setMyString, theme, setTheme }) => {
    ...
    return (
        <div className={`tw-p-4 ${theme === 'dark' ? 'tw-bg-[#252525]' : 'tw-bg-neutral-100'} tw-min-h-[100vh] tw-flex tw-flex-col`}>
            <div>
                <textarea
                    ...
                    className={`
                        tw-min-w-full 
                        ${theme === 'dark' ? 'tw-bg-[#3A3A3A]' : 'tw-bg-neutral-300'}
                        tw-min-h-[300px]
                        tw-outline-none
                        tw-drop-shadow-sm
                        tw-rounded-md
            ...
            <div className='tw-flex-wrap tw-gap-3 tw-flex tw-mt-4'>
            ...
            <div className='mt-auto tw-flex flex-wrap py-3 xl:tw-justify-start md:tw-justify-between tw-justify-between'>
                <div className='xl:tw-hidden md:tw-block block'>
                    <ToggleTheme onClick={() => setTheme((prev) => prev === 'dark' ? 'light' : 'dark')} theme={theme} />
                </div>
                <div className='xl:tw-w-full md:tw-w-1/2 tw-w-1/2 tw-flex flex-wrap'>
                    <div className='tw-flex xl:tw-w-1/2 md:tw-w-full tw-w-full xl:tw-justify-start md:tw-justify-end tw-justify-end'>
                        <span className={`tw-text-${theme === 'dark' ? 'white' : 'black'} tw-tracking-wide`}><strong className='tw-tracking-wide'>Total Words:</strong> {wordCount} words</span>
                    </div>
                    <div className='xl:tw-w-1/2 md:tw-w-full tw-w-full tw-flex xl:tw-justify-end md:tw-justify-end tw-justify-end'>
                        <span className={`tw-text-${theme === 'dark' ? 'white' : 'black'} tw-tracking-wide`}>{time} minutes need to read</span>
                    </div>
                </div>
            </div>
